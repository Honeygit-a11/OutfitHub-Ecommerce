...
const PaymentMethod = () => {
  ...
  const { getFinalAmount, deliveryCharge, discount, getCartProductDetails, getTotalCartAmount, gstRate, clearCart } = useContext(ShopContext);
  ...
  const handleCompletePayment = async () => {
    ...
    if(verified){
      try{
        ...
        // compute totals consistently with ShopContext
        const subtotal = getTotalCartAmount();
        let afterDiscount = subtotal;
        if (discount && discount > 0) {
          if (discount <= 100) afterDiscount = subtotal - (subtotal * discount) / 100;
          else afterDiscount = subtotal - discount;
        }
        const gstAmount = gstRate && gstRate > 0 ? (afterDiscount * gstRate) / 100 : 0;
        const total = Math.round((afterDiscount + gstAmount + deliveryCharge) * 100) / 100;
  ...
  await axios.post(`${API_BASE}/api/orders`,orderData);
      ...
      try { await clearCart(); } catch (e) { console.error('clearCart failed', e); }
    ...
    <div className="payment-container">
      ...
      <div className="order-summary">
        ...
        <div className="summary-row">
          ...
          <span>₹{Math.round(((getTotalCartAmount() - (discount && discount > 0 ? (discount <=100 ? getTotalCartAmount()*discount/100 : discount) : 0)) * gstRate/100) * 100) / 100}</span>
        </div>
        <hr />
        <div className="summary-row">
          <span>Discount</span>
